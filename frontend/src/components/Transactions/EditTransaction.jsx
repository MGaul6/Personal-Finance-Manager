...
import { delete_transaction, update_transaction } from "../../Actions/actions";
...
function AddAccount({
  ...
  useEffect(() => {
    if (id) {
      ...
      console.log("running", transaction, transactions, id);
    ...
    baseUrl
      ...
      .then((res) => {
        ...
        dispatch(
          update_transaction({
            id: id,
            transaction: { ...values, transaction_id: id },
          })
        );
      })
      .catch((error) => {
  ...
  const handleDelete = () => {
    baseUrl
      .delete("/transactions/id", { params: { id } })
      .then((res) => {
        setId(false);
          ...
          >
            ...
            >
              ...
              >
                {accounts.map((acc) => {
                  return (
                    <Option key={Math.random()} value={acc.account_id}>
                      {acc.name}
                    </Option>
                  );
                })}
            ...
            <Form.Item
              ...
              rules={[
                { required: true, message: "Please input Transaction Amount!" },
              ]}
            >
              <InputNumber />
            </Form.Item>
